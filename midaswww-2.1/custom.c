      ...
      List   *menu;
{
      ...
      strcpy(mnemonics,"ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890");

      XtSetArg(args[n],XmNrowColumnType,&parentType); n++;
      XtGetValues(w,args,n);
      enableMnemonics = (parentType != XmWORK_AREA); 
          ...
          if (vm->Verb != 0) 
            {
              ...
              XmString verbname; 
              char *label = XtNewString(verb->Entry);
              ...
              strtok(label,"(");   /* Remove parenthetical comments */
              for (p = label + strlen(label); *p == ' '; p--) *p='\0';  
              verbname = MidasCharToString(label);
              
              /*
               * Try to choose a suitable Mnemonic. Choose first letter of word if
               * possible, otherwise all letters in turn. 
               */
              ...
              if (Verb->IsToggle)
                {
                  ...
                  XtAddCallback(child[m++],XmNvalueChangedCallback,
                                (XtCallbackProc) CustomDispatchCommand,Verb->Command);
                 }
              else
                {
                  XtSetArg(args[n],XmNlabelString, verbname); n++;
          ...
          else if (vm->Menu != 0) 
            {
              ...
              XmString menuname; 
              char *label =  XtNewString(menu->Entry); 
              char *name = strtrn(XtNewString(menu->Entry),' ','_');
              Widget submenu = XmCreatePulldownMenu(w,name,NULL,0);

              strtok(label,"(");   /* Remove parenthetical comments */
              for (p = label + strlen(label); *--p == ' ';) *p='\0';  
              menuname = MidasCharToString(label);
 ...
 */
              ...
              if (j->Next == NULL && strcmp(label,"Help")==0)
                {
                  n = 0;
    ...
    char *Name;
{
    ...
    XrmDatabase *database = &ms->Application->Database;
    int n= 0 ;
      ...
      XmSelectionBoxCallbackStruct *cbDataP;
{
    ...
    CustomUpdateSensitivity(Header);
    CustomWrite(Header,Header->Item->Entry);
}
      ...
      Widget w;
{
    ...
    link->Widget = w;
    link->Header = Header;
    XtAddCallback(w,XmNdestroyCallback,(XtCallbackProc) CustomDisconnect,(XtPointer) connect);
}
