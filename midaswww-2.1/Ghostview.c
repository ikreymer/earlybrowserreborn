...
void GhostviewLayout(w,xfree,yfree)
    Widget w;
    Bool   xfree;
    Bool   yfree;
{
...
#define NUM_ARGS 100
    ...
    XFlush(XtDisplay(w)); /* Added by TonyJ!!!! Needed to make sure property  
                             gets installed before interpreter starts */ 
    ...
    Widget w;
{
    ...
    gvw->ghostview.busy = False;
    if (!gvw->core.being_destroyed) XDefineCursor(XtDisplay(gvw), XtWindow(gvw), gvw->ghostview.cursor);
}
    ...
    Widget w;
{
    ...
    const $DESCRIPTOR(gs_input_desc, "GS_INPUT");
    ...
    ITEM_LIST_3_T(gsi_list, 1) = {{{0, LNM$_STRING, NULL, NULL}}, 0};
	...
	*/
	...
	in_mbx_name[64] = '\0';
        gsi_list.item[0].buffer_p = in_mbx_name;
        gsi_list.item[0].buffer_size = strlen(in_mbx_name);
        ret = sys$crelnm(0, &lnt_desc, &gs_input_desc, 0, &gsi_list);
        ERR_SIGNAL(ret);
        strcat(cmd, "GS_INPUT:");
    } else {
	/*
