    ...
    FILE *file;
{
    ...
    /* Header comments (don't enforce PS-ADOBE .. TonyJ) */

    if (iscomment(line,"%!PS-Adobe-") || iscomment(line,"%!")) {
	...
	if (iscomment(line, "%%EndComments") ||
		...
		switch (sscanf(line+length("%%Pages:"), "%d %d",
		    ...
		    case 1:
                        /* protection against ludicrous maxpages added.... TonyJ */ 
                        if      (maxpages>1000) maxpages = 100;
                        else if (maxpages == 0) maxpages = 1;
    ...
    unsigned int *line_len;
{
    ...
    *line_len = strlen(line);
    if (line[0] == '\f') 
      {
        for (i=1; i < *line_len ; i++) line[i-1] = line[i];  
        *line_len--;  
      } 
